...
// $Id: gpu_matrix_vs_gpu_matrix_pitched_stdcomplexfloat_test.cpp 504 2011-07-27 09:41:14Z freiberger $
...
void multiplyMatrixVector()
{
   ...
   std::fstream myfile;
   const char * file_name = "../test/gpu_matrix_vs_gpu_matrix_pitched/Timer Results 1404/MultiplyMatrixScalar/R5C_CFloat.xls";
...
void multiplyMatrixScalar()
{
  ...
  for(unsigned test_ctr = 1; test_ctr <= 1000; test_ctr++)
  {
      ...
      for(unsigned i=0; i<2000; i++)
      {
          ...
          // \f$ y \leftarrow Ax \f$.
          scale(alpha.real(), Matrix_CUDA, ResultMatrix_CUDA);
      }
      //stop Timer and write Timer Values to std::cout and file
      timer_value = Timer.stop();
      myfile << std::endl << num_rows << "\t" << num_columns << "\t" << std::setprecision(8)<< timer_value;
      ...
      for(unsigned i=0; i<2000; i++)
      {
          // Now we can use our matrix. Perform the matrix-scalar product
          // \f$ y \leftarrow Ax \f$.
          scale(alpha.real(), Matrix_CUBLAS, ResultMatrix_CUBLAS);
      }
      //stop Timer and write Timer Values to std::cout and file
      timer_value = Timer.stop();
      myfile  << "\t" << std::setprecision(8)<< timer_value;
...
// End of $Id: gpu_matrix_vs_gpu_matrix_pitched_stdcomplexfloat_test.cpp 504 2011-07-27 09:41:14Z freiberger $.
