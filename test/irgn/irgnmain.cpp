...
*/
  ...
  for(unsigned i=0; i<num_coils;i++)
  {
    ...
    Coil_erg[i].resize(num_rows, num_columns);
  } 
...
/*
  ...
  agile::GPUMatrix<float> abs_val(num_rows,num_columns,NULL);
  agile::GPUMatrix<float> erg_val(num_rows,num_columns,0);
  std::vector<float> image_data;


  if(!fftobj_)
    fftobj_ = new agile::FFT(num_rows,num_columns);
  ...
  for(int i=0;i<num_coils;++i)
  {
    ...
    agile::absMatrix(Coil_erg[i],abs_val);                //calculate abs-val for each Coil Matrix
    agile::pow(float(2),abs_val,abs_val);                 //square all values
    agile::addMatrix(abs_val,erg_val,erg_val);            //add matrix
    std::cout<<"\n i: "<<i;
  }
  agile::sqrt(erg_val,erg_val);                   //calulate square root for final solution
  std::cout<<"\n image_data.size(): "<<image_data.size();
  erg_val.copyToHost(image_data);                  //copy solution back to host
  std::cout<<"\n image_data.size(): "<<image_data.size();
  ...
  */
  ...
  agile::IRGN<TType, TType2>* irgn_solve;
  irgn_solve = new agile::L2Solve<TType, TType2>(Coil,num_coils, irgnpara);

  irgn_solve->HighFreqPenalty();
  irgn_solve->Normalize();
  irgn_solve->Iteration();
  irgn_solve->Postprocess();

  image = irgn_solve->get_image();
...
/*
  ...
  filepath = "./"+date+"/";
  filename << "imgirgn TGV resnorm-"<<std::setprecision(4)<<(irgn_solve->get_nr_k()).back()<<".dat";
