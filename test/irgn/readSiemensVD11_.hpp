...
class VD11_Image
{
  ...
  public:
    ...
    // SET - METHODS:
    // -----------------------------
    void set_channeldata(unsigned short int channelid, std::vector<std::complex<float> > rawdata)
    {
      sChanneldata chdata;
    ...
    void set_ushLine(unsigned short int   ushLine)
    {
      ...
      ushLine_length = (ushLine >= ushLine_length) ? ushLine : ushLine_length;
    }
...
std::ostream &operator<<(std::ostream &output, const VD11_Image &image)
{
  output << image.ushLine << ' ' << image.ushAcquisition << ' ' << image.ushSlice << ' '
         ...
         << image.ushSet << ' ' << image.ushSeg << std::endl;
  return output;
}
/*
...
// =====================================================================================
  ...
  private:
    ...
    unsigned int readrawdata_line(unsigned int cPos, const sMDH& mdh_data);
...
//-------------------------------------------------------------
void ReadSiemensVD11::openfile(std::ifstream &myfile, const char * file_name)
{
...
//!  special initialize for class ReadSiemens.
// ---------------------------------------------------------------
void ReadSiemensVD11::readfile()
{
  sMDH mdh_data;

  myfile_.seekg (0, std::ios::end);
  unsigned long file_length = myfile_.tellg();
  ...
  unsigned long cPos = dat_start;

  myfile_.seekg(cPos, std::ios::beg);   //set to end of Measurement-Header = start of first Scan-Header (MDH)

  while (int(myfile_.tellg())+128 < file_length) // % fail-safe; in case we miss MDH_ACQEND
  {
...
//!
// ---------------------------------------------------------------
unsigned int ReadSiemensVD11::readrawdata_line(unsigned int cPos, const sMDH& mdh_data)
{
  unsigned szScanHeader    = 192; //% [bytes]
  unsigned szChannelHeader = 32;  //% [bytes]
  ...
  rawdata.resize(datasize,std::complex<float>(0,0));
  ...
  {
    myfile_.seekg(long(myfile_.tellg())+szChannelHeader, std::ios::beg);
