...
// $Id: fft_test.cpp 476 2011-06-16 08:54:14Z freiberger $
...
//   fftshift(kspace);
  cudafft2.inverse(kspace, img_recon);
//   fftshift(img_recon);
  fft_end_time = clock();

  // copy the result to the host
  std::vector<std::complex<float> > img_recon_host;
  img_recon.copyToHost(img_recon_host);
  end_time = clock();

  const std::complex<float>* ptr = &img_recon_host[0];
  for (unsigned row = 0; row < img_recon.getNumRows(); ++row)
  {
    // do not write the zero-padded elements
    result_file.write((char*)ptr, img_recon.getNumColumns() * sizeof(std::complex<float>));
    ptr += img_recon.getNumColumns();
  }
...
// End of $Id: fft_test.cpp 476 2011-06-16 08:54:14Z freiberger $.
