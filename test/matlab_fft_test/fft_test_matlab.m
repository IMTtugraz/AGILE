...
clear img_brain_t2;
img = img + 1i*0.1*randn(size(img)); % Just add some random complex stuff
...
writeMatlab2Bin(single(img), 'img.bin');

%% perform fft on GPU with cuda
% system('./matlab_fft_test img.bin img_recon.bin');

return

%% read in the result
% cuda_recon = readBin2Matlab('img_recon.bin', num_rows, num_columns, num_bytes_per_entry);
...
figure, imshow((abs(cuda_recon)),[]);
% figure, imshow(log(1+abs(cuda_recon)),[]);
figure,imshow(abs(matlab_img_recon-cuda_recon),[]);
