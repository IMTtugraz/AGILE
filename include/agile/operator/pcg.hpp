...
// $Id: pcg.hpp 476 2011-06-16 08:54:14Z freiberger $

#ifndef AGILE_OPERATOR_PCG_HPP
#define AGILE_OPERATOR_PCG_HPP
...
namespace agile
{
  //! \brief Conjugate Gradient method with preconditioning.
                 ...
                 TCommunicator, TForward, TPreconditioner, TBinaryMeasure> >
  {
    public:
      ...
      //! \brief Apply the PCG method to the vector x.
      //!
      ...
      //! preconditioner and \f$ F \f$ the forward operator.
      ...
      void operator() (const TVectorType& y, TVectorType& x)
      {
        ...
        m_forward(x, v_k);
        subVector(y, v_k, residual_k);

        // v_0 = P^{-1} * r_0 (i.e. solve P * v_0 = r_0);
        // v_0 is distributed before calling the preconditioner and will be
        // accumulated afterwards
        m_preconditioner(residual_k, v_k);
        ...
        for (m_iteration = 0; m_iteration < m_max_iterations; ++m_iteration)
        {
          ...
          // v_{k+1} = P^{-1} * r_{k+1}; v_k will be accumulated afterwards
          m_preconditioner(residual_k, v_k);
  ...
  //! Convenience function to create a PCG solver.
  template <typename TCommunicator, typename TForward,
            typename TPreconditioner, typename TBinaryMeasure>
  PreconditionedConjugateGradient<TCommunicator, TForward, TPreconditioner,
                                  TBinaryMeasure>
  makePCG(TCommunicator& communicator,
          const ForwardOperatorExpression<TForward>& forward,
          const InverseOperatorExpression<TPreconditioner>& preconditioner,
          const BinaryMeasureOperatorExpression<TBinaryMeasure>& measure,
          const double& rel_tolerance, const double& abs_tolerance,
          unsigned max_iterations)
  {
    return PreconditionedConjugateGradient<TCommunicator, TForward,
                                           TPreconditioner, TBinaryMeasure>(
             communicator, forward, preconditioner, measure, rel_tolerance,
             abs_tolerance, max_iterations);
  }

} // namespace agile

#endif // AGILE_OPERATOR_PCG_HPP

// End of $Id: pcg.hpp 476 2011-06-16 08:54:14Z freiberger $.
