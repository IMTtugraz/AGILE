...
#define AGILE_TEXTURE agile_matrix_texture_floatstdcomplexfloat
texture<agile::to_tuple_type<float >::texture_type> AGILE_TEXTURE;

#define AGILE_TEXTURE_2D agile_matrix_texture_2d_floatstdcomplexfloat
...
namespace lowlevel
{
  ...
  template void interpolate2d<float, std::complex<float> >(
    const float* src, unsigned numColumns, unsigned numRows,
    bool reshapeRowMajor, const std::complex<std::complex<float>>* pos,
  ...
  template void pow<float, std::complex<float>>(const float& alpha,
                              const std::complex<float>* x,
                              std::complex<float>* y, unsigned size);
  ...
  template void addVector<float, std::complex<float> >(
    const float* x, const std::complex<float>* y,
    typename promote<float, std::complex<float> >::type* z, unsigned size);

  template void divideVector<float, std::complex<float> >(
    const float& alpha, const std::complex<float>* x,
    typename promote<float, std::complex<float> >::type* y, unsigned size);

  template typename promote<float, std::complex<float> >::type getScalarProduct(
    const float* x, const std::complex<float>* y, unsigned size);

  template void multiplyConjElementwise<float, std::complex<float> >(
    const float* x, const std::complex<float>* y,
    typename promote<float, std::complex<float> >::type* z, unsigned size);

  template void multiplyElementwise<float, std::complex<float> >(
    const float* x, const std::complex<float>* y,
    typename promote<float, std::complex<float> >::type* z, unsigned size);

  template void divideElementwise<float, std::complex<float> >(
    const float* x, const std::complex<float>* y,
    typename promote<float, std::complex<float> >::type* z, unsigned size);

  template void scale<float, std::complex<float> >(
    const float& alpha, const std::complex<float>* x,
    typename promote<float, std::complex<float> >::type* y, unsigned size);
    
  template void subVector<float, std::complex<float> >(
    const float* x, const std::complex<float>* y,
    typename promote<float, std::complex<float> >::type* z, unsigned size);
  ...
  template void max<float, std::complex<float> >(
      const float* x1, const std::complex<float>* x2, typename promote<float, std::complex<float> >::type* y, unsigned size);
...
#if TType1IsComplex
#if !TType2IsComplex
template void phaseVector<float, std::complex<float> >(
    const float* x,
    std::complex<float>* y, unsigned size);
#endif  // TType1IsComplex
#endif  // !TType2IsComplex
  ...
  template void addScaledVector<float, std::complex<float>, std::complex<float> >(
    const float* x, const std::complex<float>& scale, const std::complex<float>* y,
    typename promote<typename promote<float, std::complex<float> >::type,
                     std::complex<float> >::type* z,
    unsigned size);

  template void subScaledVector<float, std::complex<float>, std::complex<float> >(
    const float* x, const std::complex<float>& scale, const std::complex<float>* y,
    typename promote<typename promote<float, std::complex<float> >::type,
                     std::complex<float> >::type* z,
    unsigned size);
