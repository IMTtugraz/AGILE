...
#define AGILE_TEXTURE agile_cs_matrix_texture_stdcomplexfloatfloat
...
namespace agile
{
  namespace detail
  {
    ...
    template void multiplyCRS<std::complex<float>, float >(
      const GPUVector<unsigned>& row_nnz, const GPUVector<unsigned>& row_offset,
      const GPUVector<unsigned>& column_index,
      const GPUVector<std::complex<float> >& matrix_data, const GPUVector<float >& x,
