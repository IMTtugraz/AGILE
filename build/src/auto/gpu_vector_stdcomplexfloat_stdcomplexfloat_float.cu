...
template void phaseVector<std::complex<float>, std::complex<float> >(
  ...
  template void addScaledVector<std::complex<float>, std::complex<float>, float >(
    const std::complex<float>* x, const std::complex<float>& scale, const float* y,
    typename promote<typename promote<std::complex<float>, std::complex<float> >::type,
                     float >::type* z,
    unsigned size);

  template void subScaledVector<std::complex<float>, std::complex<float>, float >(
    const std::complex<float>* x, const std::complex<float>& scale, const float* y,
    typename promote<typename promote<std::complex<float>, std::complex<float> >::type,
                     float >::type* z,
    unsigned size);
