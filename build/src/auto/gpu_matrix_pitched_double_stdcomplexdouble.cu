...
namespace agile
{
  template void multiply<double, std::complex<double> >(
    const GPUMatrixPitched<double >& A, const GPUVector<std::complex<double> >& x,
    GPUVector<typename promote<double, std::complex<double> >::type>& y);

  template void multiply<double, std::complex<double> >(
    const GPUVector<double >& x, const GPUMatrixPitched<std::complex<double> >& A,
    GPUVector<typename promote<double, std::complex<double> >::type>& y);

  template void multiplyElementwise<double, std::complex<double> >(
    const GPUMatrixPitched<double >& A, const GPUMatrixPitched<std::complex<double> >& B,
    GPUMatrixPitched<typename promote<double, std::complex<double> >::type>& Z);

  template void scale<double, std::complex<double> >(
    const double& alpha, const GPUMatrixPitched<std::complex<double> >& A,
    GPUMatrixPitched<typename promote<double, std::complex<double> >::type>& B);

#if !TType2IsComplex

  // second type may not be complex because, complex is already in signature
  template void interp2d<double, std::complex<double> >(
    const GPUMatrixPitched<double >& M,
    const GPUVector<std::complex<std::complex<double> > >& pos,
    GPUVector<double >& res);
