...
template void phaseVector<std::complex<float>, float >(
  ...
  template void addScaledVector<std::complex<float>, float, std::complex<float> >(
    const std::complex<float>* x, const float& scale, const std::complex<float>* y,
    typename promote<typename promote<std::complex<float>, float >::type,
                     std::complex<float> >::type* z,
    unsigned size);

  template void subScaledVector<std::complex<float>, float, std::complex<float> >(
    const std::complex<float>* x, const float& scale, const std::complex<float>* y,
    typename promote<typename promote<std::complex<float>, float >::type,
                     std::complex<float> >::type* z,
    unsigned size);
