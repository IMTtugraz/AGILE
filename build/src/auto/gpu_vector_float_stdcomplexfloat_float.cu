...
template void phaseVector<float, std::complex<float> >(
  ...
  template void addScaledVector<float, std::complex<float>, float >(
    const float* x, const std::complex<float>& scale, const float* y,
    typename promote<typename promote<float, std::complex<float> >::type,
                     float >::type* z,
    unsigned size);

  template void subScaledVector<float, std::complex<float>, float >(
    const float* x, const std::complex<float>& scale, const float* y,
    typename promote<typename promote<float, std::complex<float> >::type,
                     float >::type* z,
    unsigned size);
