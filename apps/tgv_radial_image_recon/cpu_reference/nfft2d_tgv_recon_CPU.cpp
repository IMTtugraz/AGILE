...
typedef struct problem_t {
  ...
  nfft_plan plan;
  float _Complex *nfft_g1, *nfft_g2;
...
{
  nfft_init_2d(&p->plan, p->N1, p->N2, p->nodes);

  // allocate memory for nfft_g1 nfft_g2, if necessary
  if (!p->nfft_g1) {
    p->nfft_g1 = (float _Complex*)
      nfft_malloc(p->plan.n_total*sizeof(float _Complex));
    assert(p->nfft_g1 != NULL);
    p->nfft_g2 = (float _Complex*)
      nfft_malloc(p->plan.n_total*sizeof(float _Complex));
    assert(p->nfft_g2 != NULL);
  }

  // set source and destination data pointers to our own
  p->plan.g1 = p->nfft_g1;
  p->plan.g2 = p->nfft_g2;
  nfft_fftw_init(&p->plan);

  // set nodes and precompute
  // convert node coordinates to double
  for (int j=0; j<p->nodes; j++) {
    p->plan.x[j << 1] = getf(p->coords, 0, j, 0);
    p->plan.x[(j << 1) + 1] = getf(p->coords, 1, j, 0);
  }
}
...
void mult_data_weights(array *fc, array *f, array* out)
{
    int num_nodes = fc->dim[0];
    int num_coils = fc->dim[1];

    const float* src = fc->data.f;
...
void transform(problem *p)
{
    ...
    const float* sens = p->sens.data.f; // complex
    for (int coil_counter = 0; coil_counter < num_coils; ++coil_counter)
    {
        const float* src_ptr = p->img.data.f;
    ...
    {
        p->plan.f_hat = (fftwf_complex *)&getfc(p->nfft_src, 0, 0, coil);
        p->plan.f = (fftwf_complex *)&getfc(p->nfft_dest, 0, coil, 0);
...
void transform_adjoint(problem *p)
{
...
#if SWITCH_IF_BRANCHES
    // multiply with weights
    mult_data_weights(&p->dual_v, &p->weights, &p->nfft_dest);
#else
    // copy into nfft_dest
    ...
    const float* src = p->nfft_src.data.f; // complex
    const float* sens = p->sens.data.f; // complex
    for (int counter = 0; counter < img_size; ++counter)
    {
        float sum = 0;
        const float* src_ptr = src;
        const float* sens_ptr = sens;
        for (int coil_counter = 0; coil_counter < num_coils; ++coil_counter)
        {
...
#if SWITCH_IF_BRANCHES
    // perform transform
    for (int coil = 0; coil < p->coils; ++coil)
    {
        p->plan.f_hat = (fftwf_complex *)&getfc(p->sens, 0, 0, coil);
        p->plan.f = (fftwf_complex *)&getfc(p->nfft_dest, 0, coil, 0);
        nfft_trafo_2d(&p->plan);
    }
#else
...
void h1_update_dual_v(problem *p)
{
    ...
    const float* src0 = p->nfft_dest.data.f;
    const float* src1 = p->data.data.f;
    ...
    for (int coil_counter = 0; coil_counter < p->coils; ++coil_counter)
    {
        ...
        for (int y = 0; y < y_size; ++y)
        {
            for (int x = 0; x < x_size; ++x)
            {
                ...
                float divergence = (x < x_size-2) ? *srcx : 0.0f;
                if (x >= 2)
                  divergence -= *(srcx - 2);

                divergence += (y < y_size-1) ? *srcy : 0.0f;
                if (y >= 1)
...
void h1_smooth_sens(problem *p)
{
    ...
    for (int coil_counter = 0; coil_counter < p->coils; ++coil_counter)
    {
        ...
        const float* srcx_prev = srcx - 2;
        // y == 0 && x >= 2
        for (int x = 2; x < x_size; ++x)
        {
            float sum = *srcx + *srcy + *srcx_prev;
            *dest = 0.5f * sum;
        ...
        // y > 0
        const float* srcy_prev = srcy - x_size;
        for (int y = 1; y < p->N2; ++y)
        {
            // y > 0 && x < 2
            {
                float sum = *srcx + *srcy + *srcy_prev;
                *dest = 0.5f * sum;
            ...
            for (int x = 2; x < x_size; ++x)
            {
                float sum = *srcx + *srcy + *srcx_prev + *srcy_prev;
                *dest = 0.5f * sum;
...
void update_eta(problem *p)
{
    ...
    const float* srcx = p->p_x.data.f;
    const float* srcy = p->p_y.data.f;
    ...
    // y == 0 && x > 0
    for (int x = 1; x < p->N1; ++x)
    {
        // derivative w.r.t. x
        float vxx =         *srcx - *(srcx - 1);
        float vxy = 0.5f * (*srcy - *(srcy - 1));
    ...
    for (int y = 1; y < p->N2; ++y)
    {
        ...
        {
            // derivative w.r.t. y
            float vxy = 0.5f * (*srcx - *(srcx - p->N1));
            float vyy =         *srcy - *(srcy - p->N1);
        ...
        for (int x = 1; x < p->N1; ++x)
        {
            // derivative w.r.t. x
            float vxx =         *srcx - *(srcx - 1);
            float vxy = 0.5f * (*srcy - *(srcy - 1) + *srcx - *(srcx - p->N1));
            // derivative w.r.t. y
            float vyy =         *srcy - *(srcy - p->N1);
...
void update_img(problem *p)
{
    ...
    const float* srcimg = p->img_shadow.data.f;
    const float* srcv = p->data_grad.data.f;
    const float* srcx = p->xi_x.data.f;
    const float* srcy = p->xi_y.data.f;
    float* dest = p->img.data.f;
    ...
    for (int y = 0; y < y_size; ++y)
    {
        ...
        {
            // Note: Could be optimized by moving if-branches outside loops
            float divergence = x < x_size - 1 ? *srcx : 0.0f;
            if (x >= 1)
              divergence -= *(srcx - 1);
...
void update_p(problem *p)
{
        ...
        for (int x = 0; x < x_size; ++x)
          {
            ...
            if (y < y_size - 1)
              {
                x_derivative += *(exy + x_size);
                y_derivative += *(eyy + x_size);
              }
            if (y > 0)
              {
                x_derivative -= *exy;
    ...
    destx = p->p_x.data.f;
    desty = p->p_y.data.f;
    const float *src_p_x = p->p_x_shadow.data.f;
    const float *src_p_y = p->p_y_shadow.data.f;
    const float *src_xi_x = p->xi_x.data.f;
    const float *src_xi_y = p->xi_y.data.f;
...
int main (int argc, char *argv[])
{
  ...
  // start iteration
  time_t  start_time = clock(); //start timing
  h1_minimize(&p);
  ...
  // wait
  printf("reconstruction time %f seconds\n", time1);
