function zi = test_interp2(varargin)
...
if nargin==6
    ...
    %  (1 − α)(1 − β)T[i, j]
    disp('param 1 of 4:     (1 − α)(1 − β) T[i, j]');
    [(1:length(ndx))' ndx (1-t).*(1-s)]
    
    % (1 − α) β T[i, j+1]
    disp('param 2 of 4      (1 − α) β T[i, j+1]');
    [(1:length(ndx))' ndx+1 (t).*(1-s)]

    % α(1 − β)T[i+1, j]
    disp('param 3 of 4      α(1 − β) T[i+1, j]');
    [(1:length(ndx))' ndx+nrows (1-t).*(s)]

    % αβ T[i+1, j+1]
    disp('param 4 of 4      αβ T[i+1, j+1]');
    [(1:length(ndx))' ndx+(nrows+1) (t).*(s)]
