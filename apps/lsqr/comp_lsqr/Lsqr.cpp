...
void Lsqr::do_lsqr( Model *model){
  
  float  fvec_norm2( fvec * );
  ...
  if( beta > 0.0 )
    {
      ...
      alpha = fvec_norm2( work->bidiag_wrk_vec );
    }
  
  if( alpha > 0.0 )
    {
        /* scale vector v by the inverse of ALPHA */
      fvec_scale( (1.0 / alpha), work->bidiag_wrk_vec );

        /* copy vector v to vector w */
      fvec_copy( work->bidiag_wrk_vec, work->srch_dir_vec );
    }    
...
*/      
      ...
      fvec_scale( (-alpha), input->rhs_vec );
      ...
      if( beta > 0.0 )
	{
	  ...
	  fvec_scale( (-beta), work->bidiag_wrk_vec );
...
*/
      ...
      gamma_ = sqrt( sqr(gammabar) + sqr(theta) );
      cs2 = gammabar / gamma_;
      sn2 = theta / gamma_;
...
*/
  ...
  if( input->num_rows > input->num_cols )
    temp = ( float ) ( input->num_rows - input->num_cols );

  if( sqr(input->damp_val) > 0.0 )
    temp = ( float ) ( input->num_rows );
...
void Lsqr::allocLsqrMem()
{
  ...
  return;
     ...
     /*  created with the function 'allocLsqrMem()'.                */ 
     /*                                                               */
     /*---------------------------------------------------------------*/

void Lsqr::freeLsqrMem()
{
  free_fvec(input->rhs_vec);
  free_fvec(input->sol_vec);
  free_fvec(output->std_err_vec);
  free_fvec(work->bidiag_wrk_vec);
  free_fvec(work->srch_dir_vec);
  return;
}

/*-------------------------------------------------------------------------*/
/*                                                                         */
/*  Define the function 'fvec_norm2()'.  This function takes a vector      */
...
/*-------------------------------------------------------------------------*/
/*                                                                         */
/*  Define the function 'fvec_scale()'.  This function takes a vector      */
...
void fvec_scale(float scal, fvec *vec)
{
  long   indx;
...
/*-------------------------------------------------------------------------*/
/*                                                                         */
/*  Define the function 'fvec_copy()'.  This function takes two vectors    */
...
void fvec_copy(fvec *orig, fvec *copy)
{
  long   indx;
...
void free_lvec( lvec  *lng_vec )
{
  ...
  return;
     ...
     /*---------------------------------------------------------------*/
     /*                                                               */
     /*  Define the allocation function for a float vector with       */
     /*  subscript range x[0..n-1].                                   */ 
     /*                                                               */
     /*---------------------------------------------------------------*/

fvec *alloc_fvec( long  fvec_size )
{
  fvec *float_vec;

  float_vec = (fvec *) malloc( sizeof(fvec) );
  if (!float_vec) lsqr_error("lsqr: vector allocation failure in function alloc_fvec()", -1);

  float_vec->elements = (float *) malloc( (fvec_size) * sizeof(float) );
  if (!float_vec->elements) lsqr_error("lsqr: element allocation failure in function alloc_fvec()", -1);

  float_vec->length = fvec_size;
  ...
  return float_vec;
     ...
     /*                                                               */
     /*  Define the deallocation function for the vector              */ 
     /*  created with the function 'alloc_fvec()'.                    */
     /*                                                               */
     /*---------------------------------------------------------------*/

void free_fvec( fvec  *float_vec )
{
  free((float *) (float_vec->elements));
  free((fvec *) (float_vec));
  return;
}
